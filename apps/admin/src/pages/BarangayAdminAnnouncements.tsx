...
export default function BarangayAdminAnnouncements() {
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      // Filter for this barangay only
      const barangayAnnouncements = allAnnouncements.filter((a: Announcement) =>
        a.scope === 'BARANGAY' && a.barangay_id === barangayId
      )
      setAnnouncements(barangayAnnouncements)
    } catch (err) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      data.append('scope', 'BARANGAY') // Always BARANGAY for barangay admin
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            title="No barangay announcements yet"
            description={`Create your first announcement for ${barangayName}.`}
            action={
              <button
                onClick={() => setShowCreateModal(true)}
