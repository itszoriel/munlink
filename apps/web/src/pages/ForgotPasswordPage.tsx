...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.error || 'Failed to request reset link'
      setError(msg)
    } finally {
      setSubmitting(false)
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-white px-6 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          />
          <h1 className="text-2xl font-serif font-bold text-gray-900">Reset your password</h1>
          ...
          {submitted ? (
            <div className="text-center space-y-4">
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                <Link
                  to="/login"
                  className="px-4 py-2 text-sm rounded-lg bg-ocean-600 text-white hover:bg-ocean-700"
                >
                  Back to login
                </Link>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input
                  type="email"
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-ocean-500 focus:border-transparent transition-all bg-gray-50 focus:bg-white"
