 ...
 * 
 * Valid URLs:
 * - https://... (Supabase or other CDN)
 * - data:image/... (base64)
 */
export function isLegacyPath(path?: string): boolean {
  if (!path) return false
  
  // Full URLs (Supabase, CDN, etc.) are not legacy
  if (path.startsWith('http://') || path.startsWith('https://')) {
    // But check if it's a Supabase URL (those are valid)
    return false
  }
  ...
  // Everything else is likely a legacy filesystem path
  return true
}
