...
- **Program Application Deep Review**: In Admin `Programs -> Applications`, the cards and review modal now surface applicant response fields (`application_data`) plus admin notes/rejection reason so barangay and municipal admins can review submissions more thoroughly before deciding.
...
- **Document Request Payment + Review Clarity**: Admin `Requests` now shows an explicit payment badge (`Paid`, `Unpaid`, `No Payment Required`, etc.), adds a detailed `Review Request` modal (resident input, admin edits, payment summary, supporting documents), and includes an `Auto-Generate Formal Remarks` action in `Edit / Generate PDF`.
